...
def main(args,w_array):
    ...
    model._pool_pretrained_conv = ['global'] * (
                len(args.keras_models) + len(args.numpy) + len(args.wavenet_models))
    ...
    # human data specific
    model.class_weights = [x / 100 for x in run_dataset.class_weight]
    print('building model')
    pretrained_model_path = {'keras': args.keras_models, 'numpy': args.numpy,
                             'chromwave': args.wavenet_models}
...
if __name__ == '__main__':
    ...
    # need to pause one cards otherwise there is an error in directory creation..
    ...
    seq_stratify = utils.load_json(os.path.join(args.genome_dir, 'chr_vector.json'))
    run_dataset = runtime_dataset.RuntimeDataset(f)

    nuc_preprocessing_params = {'truncate': 3,
                                'smoothing_function': 'gaussian_filter1d',
                                'normalise_read_counts': 'log2_ratio',
                                'smooth_signal': True, 'u': 2,
                                'times_median_coverage_max': 3, 'sigma': 5}
    run_dataset = runtime_dataset.RuntimeDataset(f)
    run_dataset._set_seed = 32
    run_dataset._shuffle_sequences = True
    run_dataset._include_rc = True
    run_dataset.load_existing_data = False
    run_dataset.save_data = True
    run_dataset._chr_vector = os.path.join(args.genome_dir, 'TSS_annotation.bed')
    run_dataset.preprocessing_params = [nuc_preprocessing_params]
    run_dataset._using_full_genomic_sequences = False
    run_dataset.data_format = 'processed_counts'
    run_dataset.zero_based = True
    run_dataset.parllelize = True
    ...
    run_dataset.fragment_length = None
