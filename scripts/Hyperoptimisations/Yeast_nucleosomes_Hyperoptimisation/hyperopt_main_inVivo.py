...
genome_dir = os.path.join(data_dir,'genomes/sacCer1')
...
out_dir = os.path.join(working_dir,'Test/Hyperopt/Nucleosomes')
if not os.path.exists(out_dir):
    os.makedirs(out_dir)

inVitro_model = os.path.join(model_dir,'nucleosomes','sacCer1_inVitro', 'BestWaveNet.h5')

assert os.path.exists(inVitro_model),'Error: In vitro model does not exist!'


run_dir = os.path.join(out_dir, '03_InVivo_hyperopt')
...
f = filesystem.FileSystem(genome_dir ,run_dir,source_profile= nuc_profile_inVivo,overwrite=False, test_fraction = 0.1, val_fraction=0.2,resume = True)
no_trial=len(glob.glob(f.get_output_directory()+'/hyperas*'))
print( str(no_trial)+' Trials exist, continuing with remaining')

kernel_length_choices = [16, 24,32]
pool_size_choices = [1, 2, 6, 'global']
minimum_batch_size_choices = [8,16,32]
conv_n_filters_choices = [32,64,82,128,256]
dilation_depth_choices = [9]
dilation_kernel_size_choices = [2]
weights_min_choices = [0.,0.1]#[0.0, 2.]
weights_max_choices= [0.7,0.8]  # [0.0, 2.]
...
space = { 'optimizer': choice('optimizer', ['adam', 'rmsprop','nadam']),
        ...
        'minimum_batch_size':choice('minimum_batch_size',minimum_batch_size_choices),# batch size 64 funktioniert nicht gut..
...
def train_genomic_wavenet_model(params):
    ...
    # uses pretrained in vitro model

    cmd = '-o ' + run_dir + " "+\
    '--genome_dir ' + genome_dir +" "+ \
    '--profiles ' +nuc_profile_inVivo+" "+\
...
run_dir = os.path.join(out_dir, '04_Full_Training_inVivo')
...
print( str(no_existing_trial) +' Trials existing, continuing with remaining Trials')
...
# mkdir 03_InVivo_hyperopt
# python -u hyperopt_main_inVivo.py 2> 03_InVivo_hyperopt/out.log &> 03_InVivo_hyperopt/out.log
