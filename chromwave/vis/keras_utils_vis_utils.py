...
def _check_pydot():
    ...
    if pydot is None:
        raise ImportError(
            'Failed to import `pydot_ng`. '
            'Please install `pydot_ng`. '
            'For example with `pip install pydot_ng`.')
    try:
...
def model_to_dot(model,
    ...
    for layer in layers:
        ...
        for i, node in enumerate(layer._inbound_nodes):
            ...
            if hasattr(model,'_network_nodes'):
            ...
            elif hasattr(model,'_container_nodes'):
